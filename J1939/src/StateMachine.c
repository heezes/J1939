         ...
         case  WAIT_FOR_MESSAGE:
            if ((J1939_rx_pdu->PGN == TP_CM) )//&& (J1939_rx_pdu->dest_addr == GLOBADDR || J1939_rx_pdu->dest_addr == NODEADDR))
            {
               ...
               if (J1939_rx_pdu->data[0] == TP_CM_RTS)
               {
                  ...
                  Timer_Reset((struct timer*)&J1939_rx_state_machine->t);
...
//                    J1939_rx_state_machine->timer_counter = 0;
            ...
            else if (J1939_rx_state_machine->TP == TP_CM_BAM)
            {
               if (J1939_rx_state_machine->byte_count < NUMBER_TRANS_RX_BUFFERS)
               {
                  ...
                  Timer_Set((struct timer*)&J1939_rx_state_machine->t,RECEIVE_RESP_TIMEOUT);
               }
               else
               {
                  J1939_rx_state_machine->status= WAIT_FOR_MESSAGE;
...
/*            if (J1939_rx_state_machine->timer_counter > (750/TICK))   // 750ms
            ...
            if(Timer_Expired((struct timer*)&J1939_rx_state_machine->t))
            {
         ...
         case SAVE_DATA:
            ...
            Timer_Set((struct timer*)&J1939_rx_state_machine->t, SEND_RESP_TIMEOUT);
	   ...
	   while(go_on)
	   {
		   ...
		   case TX_CHECK_CM:
			   ...
			   else
			   {
				...
				Timer_Set((struct timer*)&J1939_tx_state_machine->t, CTS_WAIT_TIMEOUT);
				J1939_tx_state_machine->status = TX_CHECK_TIMER;
				go_on = FALSE;
			   }
			   break;
		   case TX_CHECK_TIMER:
			   ...
			   if(Timer_Expired((struct timer*)&J1939_tx_state_machine->t))
			   {
				   //print_string("Timer Expired\r\n");
				   Timer_Reset((struct timer*)&J1939_tx_state_machine->t);
		   ...
		   case TX_SEND_DT:
			   if(J1939_tx_state_machine->cts_count && ((TxMsg->byte_count+6) >= J1939_tx_state_machine->packet_number*7))
			   {
				   ...
				   Transmit_J1939msg(TxMsg, J1939_tx_state_machine);
			   ...
			   else
			   {
				  ...
				  * */
				   ...
				   Timer_Set((struct timer*)&J1939_tx_state_machine->t, RECEIVE_RESP_TIMEOUT);
				   J1939_tx_state_machine->status = TX_WAIT_FOR_MESSAGE;
			   }
			   break;
		   }
...
u8 Transmit_J1939_BAM(J1939_TX_MESSAGE_T *msg, J1939_TX_STATE_MACHINE_T *J1939_tx_state_machine)
{
    ...
    if(PackFrame(&pdu) == J1939_ERROR)
       return FALSE;
    return Transmit_J1939msg(msg, J1939_tx_state_machine);
}
...
static u8 Transmit_J1939_CTS(J1939_RX_STATE_MACHINE_T* RxStateMachine)
{
    ...
    if(PackFrame(&pdu) == J1939_ERROR)
       return FALSE;
    Timer_Set((struct timer*)&RxStateMachine->t,RECEIVE_RESP_TIMEOUT);
    return TRUE;
